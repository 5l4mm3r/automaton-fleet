...
describe.skipIf(!PG_BIN)("Fleet security financial: witness capability scope (PostgreSQL)", () => {
  ...
  it("migration v6 -> v7: applied transactionally; existing agents become capability_scope 'full'; privileges stay least", async () => {
    ...
    expect((await store.migrateCheck())).toEqual({ currentVersion: 6, resultingVersion: 13, wouldApply: [7, 8, 9, 10, 11, 12, 13] });
    expect(await store.migrate()).toEqual([7, 8, 9, 10, 11, 12, 13]);
    expect(FLEET_PG_SCHEMA_VERSION).toBe(13);
    expect((await store.health()).schemaVersion).toBe(13);
