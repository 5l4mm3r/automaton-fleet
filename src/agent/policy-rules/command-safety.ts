...
const FORBIDDEN_COMMAND_PATTERNS: { pattern: RegExp; description: string }[] = [
  ...
  // Phase E (schema v10): the central ledger, payment orders/destinations, FleetAdmin instructions and the custody executor
  { pattern: /automaton-fleet-custody|custody\.env\b|\bFLEET_CUSTODY_\w+\s*=|fleet\/custody\/|\bcx_(ping|claim_instruction|report_result)\b|(?<![\w-])ledger-(model|verify|balances|journal|legacy|economics|lfc|orders|record-\w+|reverse|capital|spend-decision|withdraw|confirm|contribute|destination-\w+|estate-\w+)(?![\w-])|\bfleet_(ledger_\w+|payment_\w+|admin_\w+|economic_model|estates?|estate_\w+|assets|legacy_economics|destination_\w+|profit_contribution|order_\w+|agent_economics)\b|\bsvc_(issue_payment_instruction|expire_payment_orders)\b/i, description: "Touch the fleet central ledger, payment destinations or the custody executor" },
  // Phase F (schema v11): Genesis, capability manifests, reproduction policy, the Organisation Identity vault, knowledge promotion
  // Phase F.1: founder runtimes, their provisioner, attestation tokens and credentials
  { pattern: /automaton-fleet-founder|fleet-founders\.sh|fleet\/founder\/|automaton-founders|genesis-attest\.json|FleetFounderAttest|\/v1\/genesis\/runtime-evidence|\bFLEET_FOUNDER_\w+\s*=|runtime-instance\.json/i, description: "Touch Genesis founder runtimes, their provisioner or attestation material" },
  { pattern: /(?<![\w-])(genesis-[a-z-]+|reproduction-eligibility|knowledge-review|identity-claim-decide)(?![\w-])|\bfleet_(genesis\w*|capability_\w+|reproduction_\w+|org_identity_\w+|knowledge_review|controlled_reference\w*|agents_origin_guard|agent_can)\b|\bFLEET_CAPABILITY_MANIFEST\s*=|\bgenesis_op\b|fleet\/genesis\//i, description: "Touch fleet Genesis, capability manifests, reproduction policy or the Organisation Identity vault" },
