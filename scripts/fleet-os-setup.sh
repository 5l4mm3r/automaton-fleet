...
# Pre-Genesis step 4: the isolated research fetcher (no secrets, no database, public-web egress only).
id automaton-fleet-fetcher >/dev/null 2>&1 || run useradd --system --user-group --home-dir /nonexistent \
  --no-create-home --shell /usr/sbin/nologin --comment "Automaton fleet research fetcher" automaton-fleet-fetcher
...
run install -m 0644 -o root -g root "$REPO/deploy/systemd/automaton-fleet-fetcher.socket" /etc/systemd/system/automaton-fleet-fetcher.socket
run install -m 0644 -o root -g root "$REPO/deploy/systemd/automaton-fleet-fetcher.service" /etc/systemd/system/automaton-fleet-fetcher.service
# The fetcher may never reach this host's own addresses (e.g. its public IP → the controller on 443).
HOSTADDRS="$(ip -o addr show scope global 2>/dev/null | awk '{print $4}' | cut -d/ -f1 | sort -u | awk '{printf "%s%s", sep, $0 (index($0, ":") ? "/128" : "/32"); sep=" "}')"
run install -d -m 0755 -o root -g root /etc/systemd/system/automaton-fleet-fetcher.service.d
if [[ -n "$HOSTADDRS" ]]; then
  # The same list reaches the fetcher (it cannot enumerate interfaces itself: netlink is not an allowed address family).
  printf '# Generated by fleet-os-setup.sh: this host'"'"'s own addresses are never fetchable.\n[Service]\nIPAddressDeny=%s\nEnvironment=FLEET_FETCHER_HOST_ADDRESSES=%s\n' "$HOSTADDRS" "${HOSTADDRS// /,}" \
    | put 0644 root:root /etc/systemd/system/automaton-fleet-fetcher.service.d/host-addresses.conf
fi
