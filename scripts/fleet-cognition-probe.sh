...
#
#   sudo scripts/fleet-cognition-probe.sh [--prices <inMicrocentsPerToken>,<outMicrocentsPerToken>]
#
...
prices=""
if [[ "${1:-}" == "--prices" ]]; then
  [[ "${2:-}" =~ ^[0-9]{1,9},[0-9]{1,9}$ ]] || { echo "--prices wants <in>,<out> microcents per token" >&2; exit 2; }
  prices="$2"
fi
[[ -f "$ENVF" ]] || { echo "missing $ENVF" >&2; exit 2; }
args=()
while IFS= read -r line; do
  [[ "$line" =~ ^(FLEET_COGNITION_(PROVIDER|BASE_URL|MODEL|API_KEY_FILE|MAX_TOKENS_PARAM|ATTEMPT_TIMEOUT_MS))=([A-Za-z0-9._:/@-]{0,512})$ ]] || continue
